...
public class CategoriaDAL {
      static String obtenerCampos() {
        return "c.Id,c.Desayuno,c.PorProteina,c.PorTiempo,c.Bebidas";
    }
    ...
    private static String agregarOrderBy(Categoria pCategoria) {
        String sql = " ORDER BY c.Id,c.Desayuno,c.PorProteina,c.PorTiempo,c.Bebidas";
    ...
    public static int crear(Categoria pCategoria) throws Exception {
        ...
        try (Connection conn = comunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            sql = "INSERT INTO Categoria(Desayuno) Categoria(PorProteina)Categoria(PorTiempo)Categoria(Bebidas) VALUES(?)"; // Definir la consulta INSERT a la tabla de Rol utilizando el simbolo ? para enviar parametros
            try (PreparedStatement ps = comunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pCategoria.getDesayuno());          
                ps.setString(2, pCategoria.getPorProteina());
                ps.setString(3, pCategoria.getPorTiempo());
                ps.setString(4, pCategoria.getBebidas());// Agregar el parametro a la consulta donde estan el simbolo ? #1  
     ...
     public static int modificar(Categoria pCategoria) throws Exception {
        ...
        try (Connection conn = comunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = comunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pCategoria.getDesayuno()); // Agregar el parametro a la consulta donde estan el simbolo ? #1  
                ps.setString(2, pCategoria.getPorProteina());
                ps.setString(3, pCategoria.getPorTiempo());
                ps.setString(4, pCategoria.getBebidas()); // Agregar el parametro a la consulta donde estan el simbolo ? #1  
                ps.setInt(5, pCategoria.getId()); // Agregar el parametro a la consulta donde estan el simbolo ? #2  
    ...
    static int asignarDatosResultSet(Categoria pCategoria, ResultSet pResultSet, int pIndex) throws Exception {
        ...
        pIndex++;
        pCategoria.setDesayuno(pResultSet.getString(pIndex)); // index 2
        pIndex++;
        pCategoria.setPorProteina(pResultSet.getString(pIndex));
        pIndex++;
        pCategoria.setPorTiempo(pResultSet.getString(pIndex));
        pIndex++;
        pCategoria.setBebidas(pResultSet.getString(pIndex));
        pIndex++;
        return pIndex;
    }
    ...
    private static void obtenerDatos(PreparedStatement pPS, ArrayList<Categoria> pCategoria) throws Exception {
        try (ResultSet resultSet = comunDB.obtenerResultSet(pPS);) { // obtener el ResultSet desde la clase ComunDB
            while (resultSet.next()) { // Recorrer cada una de la fila que regresa la consulta  SELECT de la tabla Rol
                ...
                pCategoria.add(categoria); // Agregar la entidad Rol al ArrayList de Rol
            }
        ...
        if (pCategoria.getDesayuno() != null && pCategoria.getDesayuno().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" c.Desayuno LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
                statement.setString(pUtilQuery.getNumWhere(), "%" + pCategoria.getDesayuno() + "%"); 
            }
        }
         if (pCategoria.getPorProteina() != null && pCategoria.getPorProteina().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" c.PorProteina LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
                statement.setString(pUtilQuery.getNumWhere(), "%" + pCategoria.getPorProteina() + "%"); 
            }
        }
         if (pCategoria.getPorTiempo() != null && pCategoria.getPorTiempo().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" c.PorTiempo LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
                statement.setString(pUtilQuery.getNumWhere(), "%" + pCategoria.getPorTiempo() + "%"); 
            }
        }
         
         if (pCategoria.getBebidas() != null && pCategoria.getBebidas().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" c.Bebidas LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
                statement.setString(pUtilQuery.getNumWhere(), "%" + pCategoria.getBebidas() + "%"); 
            }
        }
    }
